    ...
    if mode == "adj":
        ...
        def feature_detector(board: torch.tensor) -> int:
            ...
            adj_coords = [
                ...
                if xp > -1 and xp < h and yp > -1 and yp < w
            ]
            n_hits = 0
        ...
        def feature_detector(board: torch.tensor) -> int:
            return sum(
                [
                    torch.sum((board[feature_idx, :, :] == 1).int())
                    for feature_idx in feature_idxs
                ]
            ).item()
        ...
        def feature_detector(board):
            locs_xy = sum(
                [(board[feature_idx, :, :] == 1) for feature_idx in feature_idxs]
            ).nonzero()
            locs = tuple(
                [(8 * x + y).item() for (x, y) in locs_xy]
            )  # each location is an int in range [0,63]
            return locs

    else:
        raise ValueError(
            f"Please enter a valid mode to construct a feature detector - user entered {mode}, valid modes are adj, num and loc"
        )
    return feature_detector


def make_future_feature_detector(
    feature_name: str, mode: str, steps_ahead: Optional[int] = None
) -> Callable:
    ...
    if mode == "ahead":
        ...
        def feature_detector(episode_entry: list) -> list:
            assert (
                feature_name in episode_entry[0].keys()
            ), f"Error: This feature detector has been set up to track {feature_name} which is not contained in the episode entry - please re-create it using one of the following features: {episode_entry[0].keys()}"
            episode_length = len(episode_entry)
            for trans_idx, trans_entry in enumerate(episode_entry):
                trans_entry[new_feature_name] = (
                    episode_entry[trans_idx + steps_ahead][feature_name]
        ...
        raise ValueError(
            f"User entered mode {mode}, valid modes are: ahead, traj, change"
        )
    return feature_detector
...
def generate_aug_trans(episode_entry):
    ...
    agent_loc = (
        (agent_loc - (agent_loc % 8)) // 8,
        agent_loc % 8,
    )
    agent_y, agent_x = agent_loc
    box_locs = [
        ((box_loc - (box_loc % 8)) // 8, box_loc % 8)
        for box_loc in trans["tracked_box_loc"]
    ]
    wall_locs = [
        ((wall_loc - (wall_loc % 8)) // 8, wall_loc % 8)
        for wall_loc in trans["board_state"][0]
        .view(-1)
        .topk(k=(trans["board_state"][0] == 1).to(int).sum())
        .indices
    ]
    if ((agent_y - 1, agent_x) in box_locs) and trans["action"] == 1:
        if (
            agent_y > 1
            and (agent_y - 2, agent_x) not in wall_locs
            and (agent_y - 2, agent_x) not in box_locs
        ):
            new_box_locs = [
                box_loc if box_loc != (agent_y - 1, agent_x) else (agent_y - 2, agent_x)
        ...
        if (
            agent_x > 1
            and (agent_y, agent_x - 2) not in wall_locs
            and (agent_y, agent_x - 2) not in box_locs
        ):
            new_box_locs = [
                box_loc if box_loc != (agent_y, agent_x - 1) else (agent_y, agent_x - 2)
                for box_loc in box_locs
            ]
        ...
        if (
            ...
            new_box_locs = [
                (
                    box_loc
                    if box_loc != (agent_y, agent_x + 1)
                    else (
                        (agent_y, agent_x + 2)
                        if agent_x < 6
                        else (agent_y, agent_x + 1)
                    )
                )
                for box_loc in box_locs
            ]
    ...
    def agent_info_extractor(episode_entry: list) -> list:
        ...
        for trans_idx, trans in enumerate(aug_episode_entry):
            ...
            for loc_idx in range(64):
                for future_trans_idx, future_trans in enumerate(
                    aug_episode_entry[trans_idx:-1]
                ):
                    if loc_idx in future_trans["agent_loc"] and (
                        future_trans["agent_loc"]
                        != aug_episode_entry[trans_idx + future_trans_idx + 1][
                            "agent_loc"
                        ]
                    ):  # NB: ignore no-ops and effective no-ops since want action we leave square with
                        board_locs[(loc_idx - loc_idx % 8) // 8, loc_idx % 8] = (
                            future_trans["action"]
                        )
                        break
        ...
        for trans_idx, trans in enumerate(aug_episode_entry):
            ...
            for loc_idx in range(64):
                for future_trans_idx, future_trans in enumerate(
                    ...
                    if (
                        loc_idx in future_trans["agent_loc"]
                        and aug_episode_entry[trans_idx + future_trans_idx]["agent_loc"]
                        != future_trans["agent_loc"]
                    ):
                        board_locs[(loc_idx - loc_idx % 8) // 8, loc_idx % 8] = (
                            aug_episode_entry[trans_idx + future_trans_idx]["action"]
                        )
                        break
            ...
            for loc_idx in range(64):
                for future_trans in episode_entry[trans_idx + 1 :]:
                    if loc_idx in future_trans["agent_loc"]:
                        board_locs[(loc_idx - loc_idx % 8) // 8, loc_idx % 8] += (
                            1
                            if board_locs[(loc_idx - loc_idx % 8) // 8, loc_idx % 8]
                            <= 2
                            else 0
                        )
...
def make_box_info_extractor() -> Callable:
    def box_info_extractor(episode_entry: list) -> list:
        ...
        episode_entry[0]["tracked_box_loc"] = tuple(
            [tracked_box_locs[i] for i in range(4)]
        )
        ...
        for trans_idx, trans in enumerate(aug_episode_entry[1:]):
            ...
            if (
                trans["tracked_box_loc"]
                != aug_episode_entry[trans_idx]["tracked_box_loc"]
            ):
                for i in range(4):
                    if (
                        trans["tracked_box_loc"][i]
                        != aug_episode_entry[trans_idx]["tracked_box_loc"][i]
                    ):
                        trans[
                            f"tracked_box_loc_change_after_action_{aug_episode_entry[trans_idx]['action']}"
                        ] = tuple([trans["tracked_box_loc"][i]])
            ...
            if (
                ...
                for i in range(4):
                    if (
                        ...
                        aug_episode_entry[trans_idx][
                            f"tracked_box_loc_change_with_action_{aug_episode_entry[trans_idx]['action']}"
                        ] = tuple([aug_episode_entry[trans_idx]["tracked_box_loc"][i]])
            ...
            for loc_idx in range(64):
                for future_trans in aug_episode_entry[trans_idx + 1 :]:
                    if loc_idx in future_trans["tracked_box_loc_change_after_action_1"]:
                        ...
                        loc_idx in future_trans["tracked_box_loc_change_after_action_2"]
                    ):
                        board_locs[(loc_idx - loc_idx % 8) // 8, loc_idx % 8] = 2
                        break
                    elif (
                        loc_idx in future_trans["tracked_box_loc_change_after_action_3"]
                    ):
                        board_locs[(loc_idx - loc_idx % 8) // 8, loc_idx % 8] = 3
                        break
                    elif (
                        loc_idx in future_trans["tracked_box_loc_change_after_action_4"]
                    ):
            ...
            for loc_idx in range(64):
                for future_trans in aug_episode_entry[trans_idx:]:
                    if loc_idx in future_trans["tracked_box_loc_change_with_action_1"]:
                        ...
                        loc_idx in future_trans["tracked_box_loc_change_with_action_2"]
                    ):
                        board_locs[(loc_idx - loc_idx % 8) // 8, loc_idx % 8] = 2
                        break
                    elif (
                        loc_idx in future_trans["tracked_box_loc_change_with_action_3"]
                    ):
                        board_locs[(loc_idx - loc_idx % 8) // 8, loc_idx % 8] = 3
                        break
                    elif (
                        loc_idx in future_trans["tracked_box_loc_change_with_action_4"]
                    ):
                        board_locs[(loc_idx - loc_idx % 8) // 8, loc_idx % 8] = 4
                        break
...
@torch.no_grad()
def create_probing_data(
    net: ViTBC,
    env: Env,
    flags: NamedTuple,
    num_episodes: int,
    current_board_feature_fncs: list,
    future_feature_fncs: list,
) -> list:
    """Generate a list where each entry is a dictionary of features corresponding to a single transition

    trans_entry["board_state"] = state["real_states"][0].deta
    Args:
    ...
    trans_entry = {
        feature: fnc(state["real_states"][0])
        for feature, fnc in current_board_feature_fncs
    }
    print(state["real_states"].shape)
    trans_entry["action"] = (
        net(state["real_states"].to(torch.float32)).argmax(dim=1).item()
    )
    trans_entry["board_state"] = state["real_states"][0].detach().cpu()
    trans_entry["hidden_states"] = net.get_features().detach().cpu()
    ...
    while board_num < num_episodes:
        ...
        action = net(state["real_states"].to(torch.float32)).argmax(dim=1)
        ...
        if done:
            ...
            for trans_idx, trans_entry in enumerate(episode_entry):
                ...
                trans_entry["return"] = sum(
                    [
                        (0.97**t) * future_trans["reward"]
                        for t, future_trans in enumerate(episode_entry[trans_idx:])
                    ]
                )
            ...
            board_num += 1
            print(
                "Data collected from episode",
                board_num,
                "with episode length of",
                len(episode_entry),
            )
            episode_entry = []
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="level dataset to collect transitions from",
    )
    parser.add_argument(
        "--name", type=str, default="train", help="name of dataset to create"
    )
    ...
    agent_loc_detector = make_current_board_feature_detector(
        feature_idxs=[4, 5], mode="loc"
    )
    box_loc_detector = make_current_board_feature_detector(
        feature_idxs=[2, 3], mode="loc"
    )
    tar_loc_detector = make_current_board_feature_detector(
        feature_idxs=[3, 5, 6], mode="loc"
    )
    justtar_loc_detector = make_current_board_feature_detector(
        feature_idxs=[5, 6], mode="loc"
    )
    ...
    future_feature_fncs = [
        make_future_feature_detector(feature_name="action", steps_ahead=t, mode="ahead")
        for t in range(1, 11)
    ]
    ...
    print(
        f"Dataset {args.name}_data_full_{args.model_name} contains {len(probing_data)} transitions"
    )

    if not os.path.exists("./data"):
        os.mkdir("./data")
    torch.save(
        ProbingDataset(probing_data),
        f"./data/{args.name}_data_full_{args.model_name}" + ".pt",
    )
