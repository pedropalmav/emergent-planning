...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--feature",
        type=str,
        default="agent_onto_after",
        help="square-level feature to probe for",
    )
    parser.add_argument(
        "--num_epochs", type=int, default=30, help="number of epochs to train for"
    )
    parser.add_argument(
        "--kernel", type=int, default=1, help="kernel size for local probes"
    )
    parser.add_argument(
        "--weight_decay", type=float, default=0.001, help="weight decay"
    )
    parser.add_argument(
        "--num_seeds", type=int, default=5, help="number of initialisation seeds"
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    parser.add_argument(
        ...
        action="store_false",
        default=True,
        help="use linprobe rather than convprobe",
    )
    parser.add_argument(
        "--num_layers",
        type=int,
    ...
    layers = [(f"layer{k}", k * args.channels) for k in range(args.num_layers)] + [
        ("x", 0)
    ]
    ...
    for seed in range(args.num_seeds):
        ...
        for layer_name, layer_idx in layers:
            ...
            optimiser = torch.optim.AdamW(
                params=probe.parameters(), lr=1e-3, weight_decay=args.weight_decay
            )
            ...
            for epoch in range(1, args.num_epochs + 1):
                ...
                for hiddens, states, targets in train_loader:
                    hiddens = (
                        states.to(torch.float).to(device)
                        if layer_name == "x"
                        else hiddens[:, -1, [layer_idx + c for c in channels], :, :].to(
                            device
                        )
                    )
                ...
                if epoch % 1 == 0:
                    with torch.no_grad():
                        ...
                        for hiddens, states, targets in test_loader:
                            hiddens = (
                                ...
                                if layer_name == "x"
                                    ...
                                    :, -1, [layer_idx + c for c in channels], :, :
                                ].to(device)
                            )
                            targets = targets.to(torch.long).to(device)
                            logits, loss = probe(hiddens, targets)
                            full_acc += torch.sum(
                                logits.argmax(dim=1) == targets.view(-1, 64)
                            ).item()
                        ...
                        precisions, recalls, fones, _ = precision_recall_fscore_support(
                            labs,
                            preds,
                            average=None,
                            zero_division=1,
            ...
            torch.save(
                probe.state_dict(),
                f"./results/convprobe_results/models/{args.feature}/{args.model_name }_{layer_name}_kernel{args.kernel}_wd{args.weight_decay}_seed{seed}.pt",
            )

        results_df = pd.DataFrame(results)
        results_df.to_csv(
            f"./results/convprobe_results/{args.model_name}_{args.feature}_kernel{args.kernel}_wd{args.weight_decay}_seed{seed}.csv"
        )
